...
const CloseCallReportView = React.forwardRef(({ item, usersData, projects }, ref) => {
    if (!item) return null;
    ...
    return (
        <div className="p-6 space-y-6 bg-white text-left" ref={ref}>
            <div className="border-b pb-4 border-gray-200">
                <h2 className="text-2xl font-bold text-gray-900 mb-1">Close Call Report</h2>
...
const CloseCallReportModal = ({ item, isOpen, onClose, usersData, projects }) => {
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            styleElement.textContent = `
                .close-call-export-wrapper * { color: #000000 !important; }
                /* Override for the orange boxes to keep them orange with white text */
                .close-call-export-wrapper .bg-orange-500 { background-color: #f97316 !important; color: #ffffff !important; }
                .close-call-export-wrapper .text-white { color: #ffffff !important; }
                .close-call-export-wrapper .text-orange-100 { color: #ffedd5 !important; }
                .close-call-export-wrapper .text-orange-50 { color: #fff7ed !important; }
                .close-call-export-wrapper .border-orange-600 { border-color: #ea580c !important; }
                
                /* General overrides */
                .close-call-export-wrapper .bg-gray-50 { background-color: #ffffff !important; }
                .close-call-export-wrapper .dark\\:bg-gray-800 { background-color: #ffffff !important; }
                .close-call-export-wrapper h2,
                .close-call-export-wrapper .text-gray-900,
                .close-call-export-wrapper .text-gray-500,
                .close-call-export-wrapper .text-gray-600,
                .close-call-export-wrapper .text-gray-700,
                .close-call-export-wrapper .font-medium { color: #000000 !important; }
                .close-call-export-wrapper .border-gray-200 { border-color: #e5e7eb !important; }
            `;
            document.head.appendChild(styleElement);
            element.classList.add('close-call-export-wrapper');
            ...
            link.download = `Close_Call_Report_${dateStr}.png`;
            link.click();
        } catch (error) {
            ...
            if (element) {
                element.classList.remove('close-call-export-wrapper');
            }
        }
    };
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Close Call Report">
            <div className="max-h-[80vh] overflow-y-auto">
                <CloseCallReportView item={item} usersData={usersData} projects={projects} ref={modalRef} />
            </div>
...
const CloseCallsPage = () => {
    ...
    const uniqueUserIds = useMemo(() => {
        return [...new Set(closeCalls.map(c => c.user_id))];
    }, [closeCalls]);
    ...
    const handleWizardExport = async () => {
        ...
        try {
            ...
            const itemsToExport = closeCalls.filter(item => {
            ...
            if (count > 0) {
                ...
                link.download = `Close_Call_Reports_${exportStartDate}_to_${exportEndDate}.zip`;
    ...
    const fetchData = async () => {
        setLoading(true);
        console.log('Fetching close calls data...');
        try {
            // Fetch Close Calls
            ...
            if (error) {
                console.error('Error fetching close_calls table:', error);
                throw error;
            }
            setCloseCalls(data || []);
            console.log('Close calls fetched:', data?.length);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (modalMode === 'create') {
                ...
                setCloseCalls(prev => [data[0], ...prev]);
            } else {
                const { data, error } = await supabase
                    .from('close_calls')
                    .update(payload)
                ...
                if (error) throw error;
                setCloseCalls(prev => prev.map(item => item.id === selectedItem.id ? data[0] : item));
            }

            setIsModalOpen(false);
        } catch (error) {
            console.error('Error saving close call:', error);
    ...
    const handleConfirmDelete = async () => {
        try {
            if (itemToDelete.photo_url) {
                const path = itemToDelete.photo_url.split('close-call-photos/').pop();
                if (path) {
                    await supabase.storage.from('close-call-photos').remove([path]);
                }
            }
            ...
            setCloseCalls(prev => prev.filter(item => item.id !== itemToDelete.id));
            setDeleteConfirmOpen(false);
            setItemToDelete(null);
        } catch (error) {
            console.error('Error deleting close call:', error);
    ...
    const filteredCalls = closeCalls.filter(item => 
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center h-full">
                ...
                <span className="ml-2 text-lg">Loading Close Calls...</span>
            </div>
        );
    }
    ...
    return (
        <div className="p-6 max-w-[1600px] mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div>
                    <h1 className="text-3xl font-bold text-gray-800 dark:text-white">Close Calls</h1>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
                    ...
                    <div>
                        ...
                        <Input
                            ...
                            onChange={handleInputChange}
                            placeholder="Brief title for the close call"
                            required
                        />
                    </div>
                    <div>
                        ...
                        <textarea
                            ...
                            rows="4"
                            placeholder="Describe the close call..."
                            required
                        />
                    </div>
                    <div>
            ...
            {/* Export Wizard Modal */}
            <Modal isOpen={wizardOpen} onClose={() => setWizardOpen(false)} title="Export Close Call Reports">
                <div className="p-6 space-y-4 h-[80vh] flex flex-col">
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                        Select filters to generate and export close call reports as a ZIP file.
                    </p>
            ...
            <div style={{ position: 'absolute', top: '-9999px', left: '-9999px' }}>
                {exportItemsList.map(item => (
                    <div key={item.id} id={`export-item-${item.id}`} className="w-[800px] bg-white p-4">
                        <CloseCallReportView item={item} usersData={usersData} projects={projects} />
                    </div>
                ))}
            </div>
