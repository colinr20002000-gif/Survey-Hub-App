...
serve(async (req) => {
  ...
  try {
    ...
    // Parse request body
    const {
      email, password, name, username, privilege, teamRole,
    ...
    if (!email || !password || !name || !username) {
      throw new Error('Missing required fields: email, password, name, and username are required');
    }
    ...
    if (existingUser) {
      if (existingUser.deleted_at) {
        console.log('♻️ Found soft-deleted user, will restore it');
        ...
        const { data: restoredUser, error: restoreError } = await supabaseAdmin
          ...
          .update({
            ...
            deleted_at: null // Restore the user
          })
        ...
        if (updateAuthError) {
          ...
          throw new Error(`User restored but failed to update password: ${updateAuthError.message}`);
        }
    ...
    if (listError) {
      ...
      if (existingAuthUser) {
        console.error('❌ Auth user already exists but not in users table - orphaned auth user');
