-- Add SHOW_VEHICLE_INSPECTIONS_MANAGE_BUTTON permission
-- This controls the visibility of the "Manage" button on the Vehicle Inspection page
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'privilege_permissions') THEN
        FOREACH priv_level IN ARRAY priv_levels LOOP
            ...
            VALUES (
                priv_level, 
                'SHOW_VEHICLE_INSPECTIONS_MANAGE_BUTTON', 
                'Show Manage Button', 
                'Vehicles - Vehicle Inspection', 
                (priv_level IN ('Viewer+', 'Editor', 'Editor+', 'Admin', 'Super Admin')), 
                1550
            )
