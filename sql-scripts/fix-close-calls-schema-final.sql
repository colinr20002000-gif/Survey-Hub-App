-- Unified script to fix Close Calls schema (Final Version)
-- Run this script to ensure all columns (including client) and policies are present
...
-- 4. Add client (VARCHAR) - This was missing
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'close_calls' AND column_name = 'client') THEN
        ALTER TABLE close_calls ADD COLUMN client VARCHAR(255);
    END IF;
END $$;
...
-- 6. Policies (Drop first to ensure latest definition)
