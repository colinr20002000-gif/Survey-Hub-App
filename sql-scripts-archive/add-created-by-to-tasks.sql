...
-- Add created_by column to delivery_tasks table
ALTER TABLE delivery_tasks ADD COLUMN created_by UUID REFERENCES users(id) ON DELETE SET NULL;

-- Add created_by column to project_tasks table
ALTER TABLE project_tasks ADD COLUMN created_by UUID REFERENCES users(id) ON DELETE SET NULL;

-- Create indexes for better performance on created_by lookups
CREATE INDEX idx_delivery_tasks_created_by ON delivery_tasks(created_by);
